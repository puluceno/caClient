app.controller("analysisController", ['$scope', '$timeout', 'Http', function($scope, $timeout, Http) {
	...
	this.removeLabor = function(index) {
		if ($scope.labors.length > 1)
			$scope.labors.splice(index, 1);
	};
	...
	this.clearVacations = function(index) {
		if (!$scope.showVacations) {
			$scope.labors[index].beginVacation = $scope.labors[index].endVacation = undefined;
		}
	}
		...
		if (equipment != undefined) {
			...
			Http.get("api/ca", {
				...
				.then(function successCallback(data) {
					...
					$scope.successEquipment = true;
					$scope.successMsgEquipment = "Há " + data.length + " CA para " + equipment + "."
					$timeout(function() {
						$scope.successEquipment = false;
					}, 6000);
				}, function errorCallback(data) {
	...
	this.submitAnalysis = function() {
		...
		if (this.isAnalysisFormValid()) {
			...
			fd.append("labors", angular.toJson($scope.labors));
			Http.postFormData("api/analysis", fd)
				.then(function successCallback(data) {
					...
					if (data.data === 'STARTED_WITHOUT_PROTECTION')
						$scope.errorMsg = "Não protegido na data inicial.";
					$scope.analysing = false;
					$scope.error = true;
					$timeout(function() {
						$scope.error = false;
	...
	this.isAnalysisFormValid = function() {
		for (var i = 0; i < $scope.agents.length; i++) {
			if ($scope.agents[i].startDate > $scope.agents[i].endDate) {
				$scope.errorMsgAgent = "A data inicial é maior que a data final para o  " + (i + 1) + "º agente.";
				$scope.errorAgent = true;
				$timeout(function() {
					$scope.errorAgent = false;
				}, 10000);
	...
	this.checkSelectCA = function(item, index) {
		if (item.equipment != $scope.deliveries[index].equipment) {
			$scope.deliveries[index].equipment = item.equipment;
			$scope.errorMsgEquipment = "CA " + item.number + " possui equipamento diferente do informado: " + item.equipment;
			$scope.errorEquipment = true;
			$timeout(function() {
				$scope.errorEquipment = false;
			}, 10000);
